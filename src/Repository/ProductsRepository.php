...
class ProductsRepository extends ServiceEntityRepository
{
    ...
    public function findProductsPaginated(int $page, string $slug, int $limit = 6): array
    {
        $limit = abs($limit);
        $result = [];
        $querry = $this->getEntityManager()->createQueryBuilder()->select('c','p')
            ->from('App\Entity\Products', 'p')->join('p.relation', 'c')
            ->where("c.slug = '$slug'")
            ->setMaxResults($limit)
        ...
        //Verfier si des données existent
        if(empty($data)){
            return $result;
        }

        //Nombre de pages de la categories
